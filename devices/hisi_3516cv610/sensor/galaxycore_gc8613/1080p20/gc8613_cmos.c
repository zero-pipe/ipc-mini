...
#include "gc8613_cmos.h"
#include "gc8613_cmos_param.h"
#include "gc8613_sensor_ctrl.h"

/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/
static cis_info g_gc8613_info[OT_ISP_MAX_PIPE_NUM] = {
    [0 ...(OT_ISP_MAX_PIPE_NUM - 1)] {
        ...
        .mode_tbl = {
            {
                GC8613_VMAX_LINEAR,
                GC8613_FULL_LINES_MAX_LINEAR,
                GC8613_FPS_MAX_LINEAR,
                GC8613_FPS_MIN_LINEAR,
                GC8613_WIDTH_LINEAR,
                GC8613_HEIGHT_LINEAR,
                GC8613_MODE_LINEAR,
                OT_WDR_MODE_NONE,
                "GC8613_8M_30FPS_10BIT_LINEAR_MODE"
             },
        },
        .i2c.fd = -1,
    }
...
#define GC8613_REG_VAL_TAB_ROW    26
#define GC8613_REG_VAL_TAB_COL    8

static td_u32 g_reg_val_table[GC8613_REG_VAL_TAB_ROW][GC8613_REG_VAL_TAB_COL] = {
  //  0614   0615  225   1467	 1468   b8	  b9   1447	
	{0x00, 0x00, 0x00, 0x0d, 0x0d, 0x01, 0x00, 0x77},
	{0x90, 0x02, 0x00, 0x0e, 0x0e, 0x01, 0x0a, 0x77},
	{0x01, 0x00, 0x00, 0x0e, 0x0e, 0x01, 0x1a, 0x77},
	{0x91, 0x02, 0x00, 0x0f, 0x0f, 0x01, 0x29, 0x77},
	{0x02, 0x00, 0x00, 0x0f, 0x0f, 0x01, 0x3e, 0x77},
	{0x00, 0x00, 0x00, 0x0d, 0x0d, 0x02, 0x0d, 0x75},
	{0x90, 0x02, 0x00, 0x0d, 0x0d, 0x02, 0x24, 0x75},
	{0x01, 0x00, 0x00, 0x0e, 0x0e, 0x03, 0x08, 0x75},   
	{0x91, 0x02, 0x00, 0x0e, 0x0e, 0x03, 0x2e, 0x75},
	{0x02, 0x00, 0x00, 0x0f, 0x0f, 0x04, 0x1b, 0x75},
	{0x92, 0x02, 0x00, 0x0f, 0x0f, 0x05, 0x14, 0x75},
	{0x03, 0x00, 0x00, 0x10, 0x10, 0x06, 0x17, 0x75},
	{0x93, 0x02, 0x00, 0x10, 0x10, 0x07, 0x29, 0x75},
	{0x00, 0x00, 0x01, 0x11, 0x11, 0x08, 0x13, 0x75},
	{0x90, 0x02, 0x01, 0x12, 0x12, 0x09, 0x38, 0x75},
	{0x01, 0x00, 0x01, 0x13, 0x13, 0x0c, 0x00, 0x75},
	{0x91, 0x02, 0x01, 0x14, 0x14, 0x0e, 0x35, 0x75},
	{0x02, 0x00, 0x01, 0x15, 0x15, 0x10, 0x18, 0x75},
	{0x92, 0x02, 0x01, 0x16, 0x16, 0x13, 0x29, 0x75},
	{0x03, 0x00, 0x01, 0x17, 0x17, 0x17, 0x1c, 0x75},
	{0x93, 0x02, 0x01, 0x18, 0x18, 0x1b, 0x22, 0x75},
	{0x04, 0x00, 0x01, 0x19, 0x19, 0x21, 0x04, 0x75},
	{0x94, 0x02, 0x01, 0x1b, 0x1b, 0x27, 0x18, 0x75},
	{0x05, 0x00, 0x01, 0x1d, 0x1d, 0x2f, 0x29, 0x75},
	{0x95, 0x02, 0x01, 0x1e, 0x1e, 0x39, 0x0b, 0x75},
	{0x06, 0x00, 0x01, 0x20, 0x20, 0x44, 0x0f, 0x75},
};

/****************************************************************************
  Again & Dgain table for TABLE Mode                                        *
 ****************************************************************************/
static td_u32 g_analog_gain_table[GC8613_REG_VAL_TAB_ROW] = {
    1024 ,
	1184 ,
	1440 ,
	1680 ,
    ...
    const ot_isp_sns_state *sns_state)
{
    ...
    ae_sns_dft->full_lines_max = GC8613_FULL_LINES_MAX_LINEAR;
    ae_sns_dft->hmax_times = (1000000000) / (GC8613_VMAX_LINEAR * STANDARD_FPS); /* 1000000000ns, 30fps */
    ...
    if (cis->lines_per500ms == 0) {
        ae_sns_dft->lines_per500ms = GC8613_VMAX_LINEAR * max_fps / 2; /* 30fps, div 2 */
    } else {
...
static td_void cmos_get_ae_linear_default(cis_info *cis, ot_isp_ae_sensor_default *ae_sns_dft,
    const ot_isp_sns_state *sns_state)
{
    ae_sns_dft->max_again = GC8613_AGAIN_MAX; /* 64*1024 = 65536 */
    ae_sns_dft->min_again = GC8613_AGAIN_MIN;  /* min 16 = 1024 */
    ae_sns_dft->max_again_target = ae_sns_dft->max_again;
    ae_sns_dft->min_again_target = ae_sns_dft->min_again;

    ae_sns_dft->max_dgain = GC8613_DGAIN_MAX; /* max:16X, 512 */
    ae_sns_dft->min_dgain = GC8613_DGAIN_MIN;  /* min:1X, 64 */
...
static td_void cmos_config_vmax(ot_isp_sns_state *sns_state, td_u32 vmax)
{
    ...
    sns_state->regs_info[0].i2c_data[16].data = high_8bits(GC8613_Vmax); /* index 14 */
    sns_state->regs_info[0].i2c_data[17].data = low_8bits(GC8613_Vmax);  /* index 15 */
    return;
}
...
static td_void cmos_fps_set(ot_vi_pipe vi_pipe, td_float fps, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    if ((fps > max_fps) || (fps < min_fps)) {
        isp_err_trace("ISP sensor gc8613 Not support Fps: %f\n", fps);
        return;
    }
...
static td_void cmos_slow_framerate_set(ot_vi_pipe vi_pipe, td_u32 full_lines, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    vmax = clip3(vmax, GC8613_VMAX_LINEAR, GC8613_FRAME_RATE_MIN);
	cmos_config_vmax(sns_state, vmax);
    sns_state->fl[0] = vmax;
...
static td_void cmos_inttime_update_linear(ot_isp_sns_state *sns_state, td_u32 int_time)
{
    ...
    val = (int_time > GC8613_FULL_LINES_MAX_LINEAR) ? GC8613_FULL_LINES_MAX_LINEAR : int_time;
...
static td_u32 total = sizeof(g_analog_gain_table) / sizeof(td_u32);
static td_void cmos_again_calc_table(ot_vi_pipe vi_pipe, td_u32 *pu32_again_lin, td_u32 *pu32_again_db)
{
    ...
    if (again >= g_analog_gain_table[total - 1]) {
        *pu32_again_lin = g_analog_gain_table[total - 1];
        *pu32_again_db = total - 1;
    } else {
        for (i = 1; i < total; i++) {
...
static td_void cmos_gains_update(ot_vi_pipe vi_pipe, td_u32 pu32_again, td_u32 pu32_dgain)
{
    ...
    dgain_high = (pu32_dgain >> 6) & 0xFF; /* shift 6 */
    dgain_low  = (pu32_dgain & 0x3F) ; /* shift 2 */
	
    sns_state->regs_info[0].i2c_data[2].data = 0x2d; 
    sns_state->regs_info[0].i2c_data[3].data = g_reg_val_table[pu32_again][0]; 
    sns_state->regs_info[0].i2c_data[4].data = g_reg_val_table[pu32_again][1]; 
    sns_state->regs_info[0].i2c_data[5].data = 0x28; 
    sns_state->regs_info[0].i2c_data[6].data = g_reg_val_table[pu32_again][2]; 
    sns_state->regs_info[0].i2c_data[7].data = g_reg_val_table[pu32_again][3]; 
    sns_state->regs_info[0].i2c_data[8].data = g_reg_val_table[pu32_again][4]; 
    sns_state->regs_info[0].i2c_data[9].data = g_reg_val_table[pu32_again][5]; 
    sns_state->regs_info[0].i2c_data[10].data = g_reg_val_table[pu32_again][6]; 
    sns_state->regs_info[0].i2c_data[11].data = g_reg_val_table[pu32_again][7]; 
...
static td_void cmos_isp_get_dng_default(const ot_isp_sns_state *sns_state, ot_isp_cmos_default *isp_def)
{
    ...
    switch (sns_state->img_mode) {
        case GC8613_8M_30FPS_10BIT_LINEAR_MODE:
...
static td_s32 cmos_isp_get_default(ot_vi_pipe vi_pipe, ot_isp_cmos_default *isp_def)
{
    ...
    isp_def->sns_mode.sns_id = GC8613_ID;
...
static td_void cmos_comm_sns_reg_info_init(cis_info *cis, ot_isp_sns_state *sns_state)
{
    ...
    sns_state->regs_info[0].reg_num = 18; /* reg num 17 */

    for (i = 0; i < sns_state->regs_info[0].reg_num; i++) {
        sns_state->regs_info[0].i2c_data[i].update = TD_TRUE;
        sns_state->regs_info[0].i2c_data[i].dev_addr = GC8613_I2C_ADDR;
        sns_state->regs_info[0].i2c_data[i].addr_byte_num = GC8613_ADDR_BYTE;
        sns_state->regs_info[0].i2c_data[i].data_byte_num = GC8613_DATA_BYTE;
    }

    /* Linear Mode Regs */
    sns_state->regs_info[0].i2c_data[0].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[0].reg_addr = 0x0202; // 0x0202
    sns_state->regs_info[0].i2c_data[1].delay_frame_num = 1; /* index 1 */
    sns_state->regs_info[0].i2c_data[1].reg_addr = 0x0203; // 0x0203 /* index 1 */

    /* again */
    sns_state->regs_info[0].i2c_data[2].delay_frame_num = 1; 
    sns_state->regs_info[0].i2c_data[2].reg_addr = 0x031d; 
    sns_state->regs_info[0].i2c_data[3].delay_frame_num = 1; 
    sns_state->regs_info[0].i2c_data[3].reg_addr = 0x0614;
    sns_state->regs_info[0].i2c_data[4].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[4].reg_addr = 0x0615; 
    sns_state->regs_info[0].i2c_data[5].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[5].reg_addr = 0x031d; 
    
    sns_state->regs_info[0].i2c_data[6].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[6].reg_addr = 0x0225; 
    sns_state->regs_info[0].i2c_data[7].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[7].reg_addr = 0x1467; 
    sns_state->regs_info[0].i2c_data[8].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[8].reg_addr = 0x1468; 
    sns_state->regs_info[0].i2c_data[9].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[9].reg_addr = 0x00b8; 
    sns_state->regs_info[0].i2c_data[10].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[10].reg_addr = 0x00b9; 
    sns_state->regs_info[0].i2c_data[11].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[11].reg_addr = 0x1447; 


    sns_state->regs_info[0].i2c_data[12].delay_frame_num = 1;  /* index 10 */
    sns_state->regs_info[0].i2c_data[12].reg_addr = 0x0064; // 0x00b8 /* index 10 */
    sns_state->regs_info[0].i2c_data[13].delay_frame_num = 1; /* index 11 */
    sns_state->regs_info[0].i2c_data[13].reg_addr = 0x0065; // 0x00b9 /* index 11 */



    /* vmax */
    sns_state->regs_info[0].i2c_data[14].delay_frame_num = 0; /* index 14 */
    sns_state->regs_info[0].i2c_data[14].reg_addr = 0x0340; // 0x0340 /* index 14 */
    sns_state->regs_info[0].i2c_data[15].delay_frame_num = 0; /* index 15 */
    sns_state->regs_info[0].i2c_data[15].reg_addr = 0x0341; // 0x0341 /* index 15 */


    sns_state->regs_info[0].i2c_data[16].delay_frame_num = 0; /* index 15 */
    sns_state->regs_info[0].i2c_data[16].reg_addr = 0x0259; 
    sns_state->regs_info[0].i2c_data[17].delay_frame_num = 0; /* index 16 */
    sns_state->regs_info[0].i2c_data[17].reg_addr = 0x025a; 
    return;
}
...
static td_void cmos_sns_reg_info_update(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    for (i = 0; i < sns_state->regs_info[0].reg_num; i++) {
        ...
        else if (sns_state->regs_info[0].i2c_data[i].data == sns_state->regs_info[1].i2c_data[i].data) {
            sns_state->regs_info[0].i2c_data[i].update = TD_FALSE;
        } else {
            sns_state->regs_info[0].i2c_data[i].update = TD_TRUE;
        }
    }
    //printf("####JLLEE#### F(%s)L(%d)fun(%s)update[%d]\n",__FILE__,__LINE__, __FUNCTION__, sns_state->regs_info[0].i2c_data[i].update);
      if  (    (sns_state->regs_info[0].i2c_data[3].update == TD_TRUE)
	      	|| (sns_state->regs_info[0].i2c_data[4].update == TD_TRUE)
			|| (sns_state->regs_info[0].i2c_data[6].update == TD_TRUE)
			|| (sns_state->regs_info[0].i2c_data[7].update == TD_TRUE)
			|| (sns_state->regs_info[0].i2c_data[8].update == TD_TRUE)
			|| (sns_state->regs_info[0].i2c_data[9].update == TD_TRUE)
			|| (sns_state->regs_info[0].i2c_data[10].update == TD_TRUE)
			|| (sns_state->regs_info[0].i2c_data[11].update == TD_TRUE))
	    {
			   sns_state->regs_info[0].i2c_data[2].update = TD_TRUE;
			   sns_state->regs_info[0].i2c_data[3].update = TD_TRUE;
			   sns_state->regs_info[0].i2c_data[4].update = TD_TRUE;
			   sns_state->regs_info[0].i2c_data[5].update = TD_TRUE;
			   sns_state->regs_info[0].i2c_data[6].update = TD_TRUE;
			   sns_state->regs_info[0].i2c_data[7].update = TD_TRUE;
			   sns_state->regs_info[0].i2c_data[8].update = TD_TRUE;
			   sns_state->regs_info[0].i2c_data[9].update = TD_TRUE;
	  ...
	  if  (    (sns_state->regs_info[0].i2c_data[0].update == TD_TRUE)
			|| (sns_state->regs_info[0].i2c_data[1].update == TD_TRUE))
		 {
				sns_state->regs_info[0].i2c_data[0].update = TD_TRUE;
				sns_state->regs_info[0].i2c_data[1].update = TD_TRUE;
        }
		
	 if (      (sns_state->regs_info[0].i2c_data[12].update == TD_TRUE)
			|| (sns_state->regs_info[0].i2c_data[13].update == TD_TRUE))
		{
				sns_state->regs_info[0].i2c_data[12].update = TD_TRUE;
				sns_state->regs_info[0].i2c_data[13].update = TD_TRUE;
        }
		
	 if (      (sns_state->regs_info[0].i2c_data[14].update == TD_TRUE)
			|| (sns_state->regs_info[0].i2c_data[15].update == TD_TRUE))
		{
				sns_state->regs_info[0].i2c_data[14].update = TD_TRUE;
				sns_state->regs_info[0].i2c_data[15].update = TD_TRUE;
				sns_state->regs_info[0].i2c_data[16].update = TD_TRUE;
				sns_state->regs_info[0].i2c_data[17].update = TD_TRUE;
        }
...
static td_void cmos_isp_config_image_mode_param(ot_vi_pipe vi_pipe, td_u8 sns_image_mode, ot_isp_sns_state *sns_state)
{
    ...
    switch (sns_image_mode) {
        case GC8613_8M_30FPS_10BIT_LINEAR_MODE:
            sns_state->fl_std = GC8613_VMAX_LINEAR;
            break;
        default:
            sns_state->fl_std = GC8613_VMAX_LINEAR;
            break;
...
static td_s32 cmos_isp_set_image_mode(ot_vi_pipe vi_pipe, const ot_isp_cmos_sns_image_mode *sns_image_mode)
{
    ...
    for (i = 0; i < GC8613_MODE_MAX; i++) {
        if (sns_image_mode->fps <= cis->mode_tbl[i].max_fps &&
            ...
            image_mode = (gc8613_res_mode)i;
            break;
        }
    }

    if (i >= GC8613_MODE_MAX) {
...
static void cmos_isp_init(ot_vi_pipe vi_pipe)
{
    ...
    cis->i2c.addr = GC8613_I2C_ADDR;
    cis->i2c.addr_byte_num = GC8613_ADDR_BYTE;
    cis->i2c.data_byte_num = GC8613_DATA_BYTE;
    ...
    gc8613_linear_1080p20_10bit_init(cis);
...
static td_void cmos_isp_global_init(ot_vi_pipe vi_pipe)
{
    ...
    sns_state->sync_init = TD_FALSE;
    sns_state->img_mode  = GC8613_8M_30FPS_10BIT_LINEAR_MODE;
    sns_state->wdr_mode  = OT_WDR_MODE_NONE;
    sns_state->fl_std    = GC8613_VMAX_LINEAR;
    sns_state->fl[0]     = GC8613_VMAX_LINEAR;
    sns_state->fl[1]     = GC8613_VMAX_LINEAR;
...
static void cmos_mirror_flip(ot_vi_pipe vi_pipe, ot_isp_sns_mirrorflip_type e_sns_mirror_flip)
{
  switch (e_sns_mirror_flip) {
    case ISP_SNS_NORMAL:
      cmos_write_register(vi_pipe, 0x022c,0x00);
      cmos_write_register(vi_pipe, 0x0063,0x00);
      cmos_write_register(vi_pipe, 0x0a67,0x80);
      cmos_write_register(vi_pipe, 0x0a98,0x04);
      cmos_write_register(vi_pipe, 0x05be,0x00);
      cmos_write_register(vi_pipe, 0x05a9,0x01);
      cmos_write_register(vi_pipe, 0x0a70,0x03);
      cmos_write_register(vi_pipe, 0x0a73,0x60);
      cmos_write_register(vi_pipe, 0x0a5a,0x80);
      cis_delay_ms(0x14);
      cmos_write_register(vi_pipe, 0x05be,0x01);
      cmos_write_register(vi_pipe, 0x0a70,0x00);
      cmos_write_register(vi_pipe, 0x0080,0x02);
      cmos_write_register(vi_pipe, 0x0a67,0x00);
        break;

    case ISP_SNS_MIRROR:
      cmos_write_register(vi_pipe, 0x022c,0x00);
      cmos_write_register(vi_pipe, 0x0063,0x05);
      cmos_write_register(vi_pipe, 0x0a67,0x80);
      cmos_write_register(vi_pipe, 0x0a98,0x04);
      cmos_write_register(vi_pipe, 0x05be,0x00);
      cmos_write_register(vi_pipe, 0x05a9,0x01);
      cmos_write_register(vi_pipe, 0x0a70,0x03);
      cmos_write_register(vi_pipe, 0x0a73,0x61);
      cmos_write_register(vi_pipe, 0x0a5a,0x80);
      cis_delay_ms(0x14);
      cmos_write_register(vi_pipe, 0x05be,0x01);
      cmos_write_register(vi_pipe, 0x0a70,0x00);
    ...
    case ISP_SNS_FLIP:
      cmos_write_register(vi_pipe, 0x022c,0x01);
      cmos_write_register(vi_pipe, 0x0063,0x02);
      cmos_write_register(vi_pipe, 0x0a67,0x80);
      cmos_write_register(vi_pipe, 0x0a98,0x04);
      cmos_write_register(vi_pipe, 0x05be,0x00);
      cmos_write_register(vi_pipe, 0x05a9,0x01);
      cmos_write_register(vi_pipe, 0x0a70,0x03);
      cmos_write_register(vi_pipe, 0x0a73,0x62);
      cmos_write_register(vi_pipe, 0x0a5a,0x80);
      cis_delay_ms(0x14);
      cmos_write_register(vi_pipe, 0x05be,0x01);
      cmos_write_register(vi_pipe, 0x0a70,0x00);
    ...
    case ISP_SNS_MIRROR_FLIP:
      cmos_write_register(vi_pipe, 0x022c,0x01);
      cmos_write_register(vi_pipe, 0x0063,0x07);
      cmos_write_register(vi_pipe, 0x0a67,0x80);
      cmos_write_register(vi_pipe, 0x0a98,0x04);
      cmos_write_register(vi_pipe, 0x05be,0x00);
      cmos_write_register(vi_pipe, 0x05a9,0x01);
      cmos_write_register(vi_pipe, 0x0a70,0x03);
      cmos_write_register(vi_pipe, 0x0a73,0x63);
      cmos_write_register(vi_pipe, 0x0a5a,0x80);
      cis_delay_ms(0x14);
      cmos_write_register(vi_pipe, 0x05be,0x01);
      cmos_write_register(vi_pipe, 0x0a70,0x00);
...
ot_isp_sns_obj g_sns_gc8613_1080p20_obj = {
...
{
    return &g_sns_gc8613_1080p20_obj;
}
#endif
