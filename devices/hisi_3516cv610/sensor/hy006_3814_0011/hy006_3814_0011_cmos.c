...
#include "hy006_3814_0011_cmos_param.h"
#include "hy006_3814_0011_sensor_ctrl.h"
#include "hy006_3814_0011_cmos.h"

/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/
static cis_info g_hy006_3814_0011_info[OT_ISP_MAX_PIPE_NUM] = {
    [0 ...(OT_ISP_MAX_PIPE_NUM - 1)] {
        .mutex = PTHREAD_MUTEX_INITIALIZER,
        .sns_id = HY006_3814_0011_ID,
        ...
        .mode_tbl = {
            {
                HY006_3814_0011_VMAX_2M_LINEAR,
                HY006_3814_0011_FULL_LINES_MAX_2M_LINEAR,
                HY006_3814_0011_FPS_MAX_LINEAR,
                HY006_3814_0011_FPS_MIN_LINEAR,
                HY006_3814_0011_WIDTH_2M_LINEAR,
                HY006_3814_0011_HEIGHT_2M_LINEAR,
                HY006_3814_0011_MODE_LINEAR,
                OT_WDR_MODE_NONE,
                "HY006_3814_0011_2M_30FPS_10BIT_LINEAR_MODE"
            },
            {
                HY006_3814_0011_VMAX_LINEAR,
                HY006_3814_0011_FULL_LINES_MAX_LINEAR,
                HY006_3814_0011_FPS_MAX_LINEAR,
                HY006_3814_0011_FPS_MIN_LINEAR,
                HY006_3814_0011_WIDTH_LINEAR,
                HY006_3814_0011_HEIGHT_LINEAR,
                HY006_3814_0011_MODE_LINEAR,
                OT_WDR_MODE_NONE,
                "HY006_3814_0011_4M_30FPS_10BIT_LINEAR_MODE"
            },
        },
        .i2c.fd = -1,
    }
 ...
 ****************************************************************************/
#define HY006_3814_0011_AGAIN_NODE_NUM               6
#define HY006_3814_0011_AGAIN_ADDR_INDEX_NODE_NUM    6
#define HY006_3814_0011_DGAIN_NODE_NUM               128
#define HY006_3814_0011_DGAIN_ADDR_INDEX_NODE_NUM    128

static td_u32 again_index[HY006_3814_0011_AGAIN_NODE_NUM] = {
    1024,  2048,  4096,  8192, 16384, 32768
};

static td_u32 dgain_index[HY006_3814_0011_DGAIN_NODE_NUM] = {
...
static td_u32 again_table[HY006_3814_0011_AGAIN_ADDR_INDEX_NODE_NUM] = {
    0x0, 0x8, 0x9, 0xb, 0x0f, 0x1f
};

static td_u32 dgain_table[HY006_3814_0011_DGAIN_ADDR_INDEX_NODE_NUM] = {
...
static cis_info *cmos_get_info(ot_vi_pipe vi_pipe)
{
    ...
    return &g_hy006_3814_0011_info[vi_pipe];
}
...
static td_void cmos_get_ae_linear_default(cis_info *cis, ot_isp_ae_sensor_default *ae_sns_dft,
    const ot_isp_sns_state *sns_state)
{
    ae_sns_dft->max_again = HY006_3814_0011_AGAIN_MAX; /* max 32768 */
    ae_sns_dft->min_again = HY006_3814_0011_AGAIN_MIN;  /* min 1024 */
    ae_sns_dft->max_again_target = ae_sns_dft->max_again;
    ae_sns_dft->min_again_target = ae_sns_dft->min_again;

    ae_sns_dft->max_dgain = HY006_3814_0011_DGAIN_MAX; /* max 15.75*1024 = 16128 */
    ae_sns_dft->min_dgain = HY006_3814_0011_DGAIN_MIN;  /* min 1024 */
...
static td_void cmos_fps_set(ot_vi_pipe vi_pipe, td_float fps, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    if ((fps > max_fps) || (fps < min_fps)) {
        isp_err_trace("ISP sensor hy006_3814_0011 Not support Fps: %f\n", fps);
        return;
    }
...
static td_void cmos_inttime_update_linear(cis_info *cis, ot_isp_sns_state *sns_state, td_u32 int_time)
{
    if (cis->quick_start_en == TD_TRUE && cis->i2c.fd >= 0) {
        cis_write_reg(&cis->i2c, HY006_3814_0011_EXPO_L_ADDR, lower_4bits(int_time));
        cis_write_reg(&cis->i2c, HY006_3814_0011_EXPO_M_ADDR, higher_8bits(int_time));
        cis_write_reg(&cis->i2c, HY006_3814_0011_EXPO_H_ADDR, higher_4bits(int_time));
    } else {
...
static td_void cmos_again_calc_table(ot_vi_pipe vi_pipe, td_u32 *again_lin, td_u32 *again_db)
{
    ...
    if (*again_lin >= again_index[HY006_3814_0011_AGAIN_ADDR_INDEX_NODE_NUM - 1]) {
        *again_lin = again_index[HY006_3814_0011_AGAIN_ADDR_INDEX_NODE_NUM - 1];
        *again_db = again_table[HY006_3814_0011_AGAIN_NODE_NUM - 1];
        return;
    }

    for (i = 1; i < HY006_3814_0011_AGAIN_NODE_NUM; i++) {
...
static td_void cmos_dgain_calc_table(ot_vi_pipe vi_pipe, td_u32 *dgain_lin, td_u32 *dgain_db)
{
    ...
    if (*dgain_lin >= dgain_index[HY006_3814_0011_DGAIN_ADDR_INDEX_NODE_NUM - 1]) {
        *dgain_lin = dgain_index[HY006_3814_0011_DGAIN_ADDR_INDEX_NODE_NUM - 1];
        *dgain_db = dgain_table[HY006_3814_0011_DGAIN_NODE_NUM - 1];
        return;
    }

    for (i = 1; i < HY006_3814_0011_DGAIN_NODE_NUM; i++) {
...
static td_void cmos_gains_update(ot_vi_pipe vi_pipe, td_u32 again, td_u32 dgain)
{
    ...
    if (cis->quick_start_en == TD_TRUE && cis->i2c.fd >= 0) {
        cis_write_reg(i2c, HY006_3814_0011_DGAIN_H_ADDR, reg_0x3e06);
        cis_write_reg(i2c, HY006_3814_0011_DGAIN_L_ADDR, reg_0x3e07);
        cis_write_reg(i2c, HY006_3814_0011_AGAIN_L_ADDR, reg_0x3e09);
    } else {
...
static td_void cmos_isp_get_dng_default(const ot_isp_sns_state *sns_state, ot_isp_cmos_default *isp_def)
{
    ...
    switch (sns_state->img_mode) {
        case HY006_3814_0011_4M_30FPS_10BIT_LINEAR_MODE:
            ...
            break;
        case HY006_3814_0011_2M_30FPS_10BIT_LINEAR_MODE:
            isp_def->sns_mode.dng_raw_format.bits_per_sample = DNG_RAW_FORMAT_BIT_LINEAR; /* 10bit */
            isp_def->sns_mode.dng_raw_format.white_level = DNG_RAW_FORMAT_WHITE_LEVEL_LINEAR; /* max 1023 */
            break;
        default:
...
static td_s32 cmos_isp_get_default(ot_vi_pipe vi_pipe, ot_isp_cmos_default *isp_def)
{
    ...
    isp_def->sns_mode.sns_id = HY006_3814_0011_ID;
...
static td_void cmos_comm_sns_reg_info_init(cis_info *cis, ot_isp_sns_state *sns_state)
{
    ...
    for (i = 0; i < sns_state->regs_info[0].reg_num; i++) {
        ...
        sns_state->regs_info[0].i2c_data[i].dev_addr = HY006_3814_0011_I2C_ADDR;
        sns_state->regs_info[0].i2c_data[i].addr_byte_num = HY006_3814_0011_ADDR_BYTE;
        sns_state->regs_info[0].i2c_data[i].data_byte_num = HY006_3814_0011_DATA_BYTE;
    }

    /* Linear Mode Regs */
    sns_state->regs_info[0].i2c_data[EXPO_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_L_IDX].reg_addr = HY006_3814_0011_EXPO_L_ADDR;
    sns_state->regs_info[0].i2c_data[EXPO_M_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_M_IDX].reg_addr = HY006_3814_0011_EXPO_M_ADDR;
    sns_state->regs_info[0].i2c_data[EXPO_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_H_IDX].reg_addr = HY006_3814_0011_EXPO_H_ADDR;

    sns_state->regs_info[0].i2c_data[AGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[AGAIN_L_IDX].reg_addr = HY006_3814_0011_AGAIN_L_ADDR;

    sns_state->regs_info[0].i2c_data[DGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[DGAIN_H_IDX].reg_addr = HY006_3814_0011_DGAIN_H_ADDR;
    sns_state->regs_info[0].i2c_data[DGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[DGAIN_L_IDX].reg_addr = HY006_3814_0011_DGAIN_L_ADDR;

    sns_state->regs_info[0].i2c_data[VMAX_L_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[VMAX_L_IDX].reg_addr = HY006_3814_0011_VMAX_L_ADDR;
    sns_state->regs_info[0].i2c_data[VMAX_H_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[VMAX_H_IDX].reg_addr = HY006_3814_0011_VMAX_H_ADDR;
    return;
}
...
static td_s32 cmos_isp_get_standby_cfg(ot_vi_pipe vi_pipe, ot_isp_sns_regs_info *standby_cfg)
{
    ...
    ret = hy006_3814_0011_get_standby_cfg(standby_cfg);
    if (ret == TD_SUCCESS) {
    ...
    ot_isp_sns_state *sns_state)
{
    ...
    switch (sns_image_mode) {
        case HY006_3814_0011_4M_30FPS_10BIT_LINEAR_MODE:
            sns_state->fl_std = HY006_3814_0011_VMAX_LINEAR;
            break;
        case HY006_3814_0011_2M_30FPS_10BIT_LINEAR_MODE:
            sns_state->fl_std = HY006_3814_0011_VMAX_2M_LINEAR;
            break;
        default:
            sns_state->fl_std = HY006_3814_0011_VMAX_LINEAR;
            break;
...
static td_s32 cmos_isp_set_image_mode(ot_vi_pipe vi_pipe, const ot_isp_cmos_sns_image_mode *sns_image_mode)
{
    ...
    for (i = 0; i < HY006_3814_0011_MODE_MAX; i++) {
        if (sns_image_mode->fps <= cis->mode_tbl[i].max_fps &&
            ...
            image_mode = (hy006_3814_0011_res_mode)i;
            break;
        }
    }

    if (i >= HY006_3814_0011_MODE_MAX) {
...
static void cmos_isp_init(ot_vi_pipe vi_pipe)
{
    ...
    cis->i2c.addr = HY006_3814_0011_I2C_ADDR;
    cis->i2c.addr_byte_num = HY006_3814_0011_ADDR_BYTE;
    cis->i2c.data_byte_num = HY006_3814_0011_DATA_BYTE;
    ...
    hy006_3814_0011_linear_4m30_10bit_init(cis);
...
static td_void cmos_isp_global_init(ot_vi_pipe vi_pipe)
{
    ...
    sns_state->sync_init = TD_FALSE;
    sns_state->img_mode  = HY006_3814_0011_4M_30FPS_10BIT_LINEAR_MODE;
    sns_state->wdr_mode  = OT_WDR_MODE_NONE;
    sns_state->fl_std    = HY006_3814_0011_VMAX_LINEAR;
    sns_state->fl[0]     = HY006_3814_0011_VMAX_LINEAR;
    sns_state->fl[1]     = HY006_3814_0011_VMAX_LINEAR;
...
static td_void cmos_isp_set_pixel_detect(ot_vi_pipe vi_pipe, td_bool enable)
{
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_2To1_LINE) {
        ...
        if (sns_state->img_mode == HY006_3814_0011_4M_30FPS_10BIT_LINEAR_MODE) {
            full_lines_5fps = (HY006_3814_0011_VMAX_LINEAR * STANDARD_FPS) / 5; /* 30fps, 5fps */
        } else if (sns_state->img_mode == HY006_3814_0011_2M_30FPS_10BIT_LINEAR_MODE) {
            full_lines_5fps = (HY006_3814_0011_VMAX_2M_LINEAR * STANDARD_FPS) / 5; /* 30fps, 5fps */
        } else {
            return;
        }
    }
    ...
    if (enable) { /* setup for ISP pixel calibration mode */
        cis_write_reg(i2c, HY006_3814_0011_AGAIN_L_ADDR, 0x00);

        cis_write_reg(i2c, HY006_3814_0011_DGAIN_H_ADDR, 0x00);
        cis_write_reg(i2c, HY006_3814_0011_DGAIN_L_ADDR, 0x80);

        cis_write_reg(i2c, HY006_3814_0011_VMAX_L_ADDR, low_8bits(full_lines_5fps));
        cis_write_reg(i2c, HY006_3814_0011_VMAX_H_ADDR, high_8bits(full_lines_5fps));

        cis_write_reg(i2c, HY006_3814_0011_EXPO_L_ADDR, lower_4bits(full_lines_5fps - FL_OFFSET_LINEAR));
        cis_write_reg(i2c, HY006_3814_0011_EXPO_M_ADDR, higher_8bits(full_lines_5fps - FL_OFFSET_LINEAR));
        cis_write_reg(i2c, HY006_3814_0011_EXPO_H_ADDR, higher_4bits(full_lines_5fps - FL_OFFSET_LINEAR));
    } else { /* setup for ISP 'normal mode' */
        lines_max = cis->mode_tbl[sns_state->img_mode].max_ver_lines;
        sns_state->fl_std = (sns_state->fl_std > lines_max) ? lines_max : sns_state->fl_std;
        cis_write_reg(i2c, HY006_3814_0011_VMAX_L_ADDR, low_8bits(sns_state->fl_std));
        cis_write_reg(i2c, HY006_3814_0011_VMAX_H_ADDR, high_8bits(sns_state->fl_std));
        sns_state->sync_init = TD_FALSE;
    }
    return;
}
...
ot_isp_sns_obj g_sns_hy006_3814_0011_obj = {
...
{
    return &g_sns_hy006_3814_0011_obj;
}
